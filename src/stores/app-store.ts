...
export default class AppStore {
    ...
    getErrorForNonEuClients = () => ({
        ...
        title: localize('FrostyDBot is unavailable for this account'),
    ...
    getErrorForEuClients = (is_logged_in = false, country: string | undefined = undefined) => {
        return {
            text: ' ',
            title: is_logged_in
                ? localize(`FrostyDBot is not available for ${country || 'EU'} clients`)
                : localize(`FrostyDBot is unavailable in ${country || 'the EU'}`),
